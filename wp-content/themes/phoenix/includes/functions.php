...
function dh_get_main_col_class($layout='',$body = false,$priority = 10){
	...
	if(!$body){
		if($layout == 'full-width'){
			...
			$col_class = 'col-md-6';
			add_action('dh_right_sidebar','dh_get_sidebar',$priority);
			add_action('dh_left_sidebar_extra','dh_get_extra_sidebar',$priority);
		}elseif ($layout == 'two-left-sidebar'){
			$col_class = 'col-md-7';
			global $dh_extra_column;
				$dh_extra_column=2;
			add_action('dh_left_sidebar','dh_get_sidebar',$priority);
			add_action('dh_left_sidebar_extra','dh_get_extra_sidebar',$priority);
			...
			global $dh_extra_column;
				$dh_extra_column=2;
			add_action('dh_right_sidebar','dh_get_sidebar',$priority);
			add_action('dh_right_sidebar_extra','dh_get_extra_sidebar',$priority);
		}
	}
	if($body){
		if(empty($layout))
...
function dh_enqueue_google_font(){
	...
	foreach ($typography_arr as $font){
		...
		if(!empty($typography['font-family'])){
			...
			wp_enqueue_style( 'dh-'.sanitize_title($font_family).'-'.$font_style, "$protocol://fonts.googleapis.com/css?family=$font_family:$font_style$subset",false);
		}
	}
}
	...
	<div class="share-links">
		...
		<div class="share-icons">
			...
			<span class="facebook-share">
				<a href="<?php echo esc_url('http://www.facebook.com/sharer.php?u='.get_the_permalink()) ?>" onclick="javascript:window.open(this.href,'', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=220,width=600');return false;" title="<?php echo __('Facebook',DH_DOMAIN)?>"><i class="fa fa-facebook<?php echo ($outlined ? ' facebook-outlined':'')?>"></i></a>
			</span>
			<?php endif;?>
			<?php if($twitter):?>
			<span  class="twitter-share">
				<a href="<?php echo esc_url('https://twitter.com/share?url='.get_the_permalink()) ?>" onclick="javascript:window.open(this.href,'', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=260,width=600');return false;" title="<?php echo __('Twitter',DH_DOMAIN)?>"><i class="fa fa-twitter<?php echo ($outlined ? ' twitter-outlined':'')?>"></i></a>
			</span>
			<?php endif;?>
			<?php if($google):?>
			<span class="google-plus-share">
				<a href="<?php echo esc_url('https://plus.google.com/share?url='.get_the_permalink()) ?>" onclick="javascript:window.open(this.href,'', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;" title="<?php echo __('Google +',DH_DOMAIN)?>"><i class="fa fa-google-plus<?php echo ($outlined ? ' google-plus-outlined':'')?>"></i></a>
			</span>
			<?php endif;?>
			<?php if($pinterest):?>
			<span class="pinterest-share">
				<a href="<?php echo esc_url('http://pinterest.com/pin/create/button/?url='.get_the_permalink().'&media='.(function_exists('the_post_thumbnail') ? wp_get_attachment_url(get_post_thumbnail_id()):'').'&description='.get_the_title()) ?>" title="<?php echo __('pinterest',DH_DOMAIN)?>"><i class="fa fa-pinterest<?php echo ($outlined ? ' pinterest-outlined':'')?>"></i></a>
			</span>
			<?php endif;?>
			<?php if($linkedin):?>
			<span class="linkedin-share">
				<a href="<?php echo esc_url('http://www.linkedin.com/shareArticle?mini=true&url='.get_the_permalink().'&title='.get_the_title())?>" onclick="javascript:window.open(this.href,'', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;" title="<?php echo __('Linked In',DH_DOMAIN)?>"><i class="fa fa-linkedin<?php echo ($outlined ? ' linkedin-outlined':'')?>"></i></a>
			</span>
			<?php endif;?>
		</div>
	</div>
...
<div class="related-post">
	<div class="related-post-title">
		<h3><span><?php echo __("You may also like",DH_DOMAIN)?></span></h3>
	</div>
	<div class="row related-post-items">
		<?php while ($related->have_posts()): $related->the_post();global $post;?>
			<div class="related-post-item col-md-4 col-sm-6">
				...
				<?php dh_post_meta(true,false,false,false,true,', ')?>
				</div>
				...
				<?php dh_post_meta(false,true,false,true,true,', ')?>
				</div>
					...
					<header class="comment-header">
						<div class="comment-info">
							...
							<span class="ispostauthor">
								<?php _e('Author', DH_DOMAIN); ?>
							</span>
							<?php endif; ?>
							<span class="comment-meta">
								<time datetime="<?php echo get_comment_time('c'); ?>">
									<?php echo sprintf(__('%1$s at %2$s', DH_DOMAIN) , get_comment_date() , get_comment_time()); ?>
								</time>
								<span class="comment-edit">
									<?php edit_comment_link('' . __('Edit', DH_DOMAIN)); ?>
								</span>
							</span>
						</div>
						<?php if ('0' == $comment->comment_approved): ?>
							<p class="comment-pending"><?php _e('Your comment is awaiting moderation.', DH_DOMAIN); ?></p>
						<?php endif; ?>
					</header>
...
function dh_get_search_form($ajax=true){
	$search_form = '<form method="GET" class="searchform search-ajax" action="'.esc_url( home_url( '/' ) ).'" role="form">
					<input type="search" class="searchinput" name="s" autocomplete="off" value="" placeholder="'.__( 'Search...', DH_DOMAIN ).'" />
					<input type="submit" class="searchsubmit hidden" name="submit" value="'.__( 'Search', DH_DOMAIN ).'" />
...
function dh_comment_form( $args = array(), $post_id = null ) {
	...
	<div id="respond-wrap">
		<?php 
			...
			$fields =  array(
				'author' => '<div class="row"><p class="comment-form-author col-sm-4"><label for="author">' . __( 'Name', DH_DOMAIN ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) . '<input id="author" name="author" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
				'email' => '<p class="comment-form-email col-sm-4"><label for="email">' . __( 'Email', DH_DOMAIN ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) . '<input id="email" name="email" type="text" class="form-control" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>',
				'url' => '<p class="comment-form-url col-sm-4"><label for="url">' . __( 'Website', DH_DOMAIN ) . '</label><input id="url" name="url" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p></div>'
			);
			$comments_args = array(
					'fields'               => apply_filters( 'comment_form_default_fields', $fields ),
					'logged_in_as'		   => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', DH_DOMAIN ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>',
					'title_reply'          => sprintf('<span>%s</span>',__( 'Leave a reply', DH_DOMAIN )),
					'title_reply_to'       => sprintf('<span>%s</span>',__( 'Leave a reply to %s', DH_DOMAIN )),
					'cancel_reply_link'    => __( 'Click here to cancel the reply', DH_DOMAIN ),
					'label_submit'         => __( 'Submit', DH_DOMAIN ),
					'comment_field'		   => '<p class="comment-form-comment"><label for="comment">' . __( 'Comment', DH_DOMAIN ) . '</label><textarea class="form-control" id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
					'must_log_in'		   => '<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.', DH_DOMAIN ), wp_login_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>',
			);
...
function dh_nth_word($text, $nth = 1, $echo = true,$is_typed = false,$typed_color = ''){
	...
	if ( 'characters' == _x( 'words', 'word count: words or characters?',DH_DOMAIN) && preg_match( '/^utf\-?8$/i', get_option( 'blog_charset' ) ) ) {
...
function dh_post_nav() {
	...
	<nav class="post-navigation" role="navigation">
		<div class="row">
			<?php $prev_link = get_previous_post_link( '%link', _x( '%title', 'Previous post link', DH_DOMAIN ) ); ?>
			<div class="col-sm-6">
			<?php if($prev_link):?>
				<div class="prev-post">
					<span>
					<?php echo __('Previous article',DH_DOMAIN)?>
					</span>
					<?php echo dhecho($prev_link)?>
				</div>
			<?php endif;?>
			</div>
			<?php $next_link = get_next_post_link( '%link', _x( '%title', 'Next post link', DH_DOMAIN ) ); ?>
			<div class="col-sm-6">
			<?php if(!empty($next_link)):?>
				<div class="next-post">
					<span>
						<?php echo __('Next article',DH_DOMAIN)?>
					</span>
					<?php echo dhecho($next_link)?>
				</div>
			<?php endif;?>
			</div>
		</div>
...
function dh_post_meta($show_date=true,$show_comment = true,$show_category= true,$show_author = true,$echo = true,$meta_separator= ', ',$date_format = null,$icon = false) {
	...
	if($show_author){
		...
		if($icon)
			$author_html .= '<i class="fa fa-pencil-square-o"></i>';//__('By', DH_DOMAIN);
		$author = sprintf(
			'<a href="%1$s" title="%2$s" rel="author">%3$s</a>',
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ), get_the_author_meta( 'nicename' ) ) ),
			esc_attr( sprintf( __( 'Posts by %s', DH_DOMAIN), get_the_author() ) ),
			get_the_author()
		);
		$author_html .= sprintf(__('By %1$s', DH_DOMAIN),$author);
		$author_html .= '</span>';
		$html[] = $author_html;
	}
	// Date
	...
	if($show_category){
		...
		$categories_html .= sprintf(__('In %1$s',DH_DOMAIN),get_the_category_list(', '));
	...
	if (comments_open()) {
		...
		if (get_comments_number() == 0) {
			$comment_title = sprintf(__('Leave a comment on: &ldquo;%s&rdquo;', DH_DOMAIN) , get_the_title());
			$comment_number = '0 '.__('Comment', DH_DOMAIN);
		} else if (get_comments_number() == 1) {
			$comment_title = sprintf(__('View a comment on: &ldquo;%s&rdquo;', DH_DOMAIN) , get_the_title());
			$comment_number = '1 ' . __('Comment', DH_DOMAIN);
		} else {
			$comment_title = sprintf(__('View all comments on: &ldquo;%s&rdquo;', DH_DOMAIN) , get_the_title());
			$comment_number =  get_comments_number() . ' ' . __('Comments', DH_DOMAIN);
		}
...
function dh_paginate_links_short($args = array(), $query = null){
	...
	$defaults = array(
		...
		'add_fragment' => '',
		'add_args'=>array(),
		'before' => '<div class="paginate"><div class="paginate_links"><span class="pagination-meta">'.sprintf(__("%d/%d", DH_DOMAIN), $paged, $max_num_pages).'</span>',
		'after' => '</div></div>',
		'echo' => true,
	);
...
function dh_morphsearchform(){
	...
	<div class="morphsearch" id="morphsearch">
		<form class="morphsearch-form" method="get"  action="<?php echo esc_url( home_url( '/' ) ) ?>" role="form">
			<input type="search" name="s" placeholder="<?php _e('Search...',DH_DOMAIN)?>" class="morphsearch-input">
		...
		<div class="morphsearch-content<?php echo (defined( 'WOOCOMMERCE_VERSION' )  ? ' has-3colum':'') ?>">
			...
			<div class="dummy-column">
				<h2><?php _e('Product',DH_DOMAIN) ?></h2>
				<?php 
				$query_args = array(
		    		'posts_per_page' => 6,
		    		'post_status' 	 => 'publish',
				...
				if ( $r->have_posts() ) {
					while ( $r->have_posts() ) {
						...
						<a href="<?php the_permalink()?>" class="dummy-media-object">
							<?php echo dhecho($product->get_image('dh-thumbnail-square')); ?>
							<div>
								<h3><?php echo dhecho($product->get_title()); ?></h3>
								<?php if ( ! empty( $show_rating ) ) echo dhecho($product->get_rating_html()); ?>
								<div class="price">
									<?php echo dhecho($product->get_price_html()); ?>
								</div>
							</div>
						</a>
						<?php
			...
			<?php }?>
			<div class="dummy-column">
				<h2><?php _e('Popular',DH_DOMAIN) ?></h2>
				<?php 
				...
				<a href="<?php the_permalink()?>" class="dummy-media-object">
					...
					<div>
						<h3><?php the_title()?></h3>
						<?php echo '<span>'.sprintf(__('%s Comment',DH_THEME_DOMAIN),get_comments_number()).'</span>'; ?>
					</div>
				</a>
				<?php endwhile; ?>
				<?php 
...
function dh_mailchimp_form($echo = true){
	ob_start();
	?>
	<form method="get" class="mailchimp-form" action="<?php echo home_url('/')?>">
		<?php 
		...
		<div class="mailchimp-form-result">
			...
			if($mailchimp_subscribe == -1){
				echo '<span class="error">'.__( 'Not Subscribe to Mailchimp!', DH_DOMAIN ).'<span>';
			}elseif ($mailchimp_subscribe == 1){
				echo '<span class="success">'.__( 'Subscribe to Mailchimp Successful!', DH_DOMAIN ).'</span>';
			}
			?>
		</div>
		<?php endif;?>
		<div class="mailchimp-form-content clearfix">
			<div class="row">
				<div class="col-sm-8">
					<label for="" class="hide"><?php _e('Subscribe',DH_DOMAIN)?></label>
					<input type="email" class="form-control"  required="required" autocomplete="off" placeholder="<?php _e('Enter your email...',DH_DOMAIN)?>" name="email">
				</div>
				<div class="col-sm-4">
					<button type="submit" class="btn btn-primary"><?php _e('Subscribe',DH_DOMAIN)?></button>
				</div>
			</div>
...
function dh_landing_hero_content(){
	...
	<div class="row">
		<div class="col-sm-4">
			<div class="hero-icon">
				<span class="svg-icon ux-design-icon svg-appear">
					<i class="et-line-browser"></i>
				</span>
				<span class="svg-text svg-ux-design-text svg-appear"><?php _e('UX Design',DH_DOMAIN)?></span>
			</div>
		</div>
		<div class="col-sm-4">
			<div class="hero-icon">
				<span class="svg-icon branding-icon svg-appear">
					<i class="et-line-tools"></i>
				</span>
				<span class="svg-text svg-branding-text svg-appear"><?php _e('Branding',DH_DOMAIN)?></span>
			</div>
		</div>
		<div class="col-sm-4">
			<div class="hero-icon">
				<span class="svg-icon web-dev-icon svg-appear">
					<i class="et-line-laptop"></i>
				</span>
				<span class="svg-text svg-web-dev-text svg-appear"><?php _e('Web Dev',DH_DOMAIN)?></span>
				</div>
			</div>
		</div>
		<div class="hero-link svg-appear">
        	<a href="#"><?php _e('See our work',DH_DOMAIN)?><i class="fa fa-angle-right"></i></a>				 
        </div>
	<?php
