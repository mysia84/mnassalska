		...
		/* If viewing any type of archive. */
			...
			if ( is_category() || is_tag() || is_tax() ) {
				...
				if(is_numeric(get_query_var('w') && 0 !== get_query_var('w') ))
					$item['last'] =  sprintf( __('Weekly Archive for: "%s"',DH_DOMAIN),get_the_time('W'));
				elseif ( is_day() )
				$item['last'] = sprintf( __('Daily Archive for: "%s"',DH_DOMAIN),get_the_time('F jS, Y'));
				elseif ( is_month() )
				$item['last'] =  sprintf( __('Monthly Archive for: "%s"',DH_DOMAIN),get_the_time('F jS, Y'));
				elseif ( is_year() )
				$item['last'] =  sprintf(__('Yearly Archive for: "%s"',DH_DOMAIN),get_the_time('Y'));
			}
		
			else if ( is_author() )
				$item['last'] =  sprintf(__('Author Archive for: "%s"',DH_DOMAIN),get_the_author_meta( 'display_name', $wp_query->post->post_author ));
		}
		
		/* If viewing search results. */
		else if ( is_search() )
			$item['last'] =  __('Search',DH_DOMAIN);//sprintf(__('search Results for: "%s"',DH_DOMAIN),stripslashes( strip_tags( get_search_query() ) ));
		
		/* If viewing a 404 error page. */
		else if ( is_404() )
			$item['last'] = __( 'Page Not Found', DH_DOMAIN );
