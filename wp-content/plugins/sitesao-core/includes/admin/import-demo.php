...
class DHImportDemo{
	...
	public function admin_menu(){
		add_submenu_page( 'themes.php' , __('Import Demo',DH_DOMAIN) , __('Import Demo',DH_DOMAIN) , 'manage_options' , 'import-demo' , array(&$this,'output') );
	}
	...
	public function output(){
		...
		<div class="dh-message content" style="display:none;">
			...
			<h1 class="dh-message-title"><?php esc_html_e('Importing Demo Content...',DH_DOMAIN)?></h1>
			<p class="dh-message-text"><?php _e('Please be patient and do not navigate away from this page while the import is in&nbsp;progress. This can take a while if your server is slow (inexpensive hosting).<br>You will be notified via this page when the import is completed.',DH_DOMAIN)?></p>
		</div>
		<div class="dh-message error" style="display:none;">
			<h1 class="dh-message-title"><?php esc_html_e('Error has occured',DH_DOMAIN)?></h1>
			<p class="dh-message-text"></p>
		</div>
		<div class="dh-message success" style="display:none;">
			<h1 class="dh-message-title"><?php esc_html_e('Import completed successfully!',DH_DOMAIN)?></h1>
			<p class="dh-message-text"><?php _e('Now you can see the result at',DH_DOMAIN)?> <a href="<?php echo site_url(); ?>" target="_blank"><?php _e('your site',DH_DOMAIN)?></a><br><?php _e('or start customize via',DH_DOMAIN)?> <a href="<?php echo admin_url('themes.php?page=theme-options'); ?>"><?php _e('Theme Options',DH_DOMAIN)?></a></p>
		</div>

		<form class="dh-importer" action="?page=import-demo" method="post">
			<h1 class="dh-importer-title"><?php _e('Import Demo Data',DH_DOMAIN)?></h1>
			...
			<div class="dh-importer-options">
				<div class="dh-importer-option demo-data" style="display: none">
					<label class="dh-importer-option-check">
						...
						<span class="dh-importer-title"><?php _e('Import Demo Data',DH_DOMAIN)?></span>
					</label>
				</div>
				<div class="dh-importer-note">
					<strong><?php esc_html_e('Important Notes:',DH_DOMAIN)?></strong>
					<ol>
						<li><?php _e('We recommend to run Demo Import on a clean WordPress installation.',DH_DOMAIN)?></li>
						<li><?php echo sprintf(__('To reset your installation we recommend %s plugin.',DH_DOMAIN),'<a href="http://wordpress.org/plugins/wordpress-database-reset/" target="_blank">Wordpress Database Reset</a>')?></li>
						<li><?php _e('The Demo Import will not import the images we have used in our live demo, due to copyright/license reasons.',DH_DOMAIN)?></li>
						<li><?php _e('Do not run the Demo Import multiple times one after another, it will result in double content.',DH_DOMAIN)?></li>
					</ol>
				</div>
				<input type="hidden" name="action" value="import-demo">
				<input class="button-primary" id="run_import_demo_data" type="submit" value="<?php _e('Import',DH_DOMAIN)?>">
			</div>
		</form>
		<script>
				...
				foreach($menus as $menu)
				{
					if(is_object($menu))
					{
						...
						if(isset( $one_page ) && $one_page->ID && $menu->name == 'One Page Menu') {
							update_post_meta($one_page->ID, '_dh_main_menu', $menu->term_id);
						}
					}
				}
			}
