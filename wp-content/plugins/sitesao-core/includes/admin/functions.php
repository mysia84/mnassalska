			...
			switch ($field['type']){
				...
				case 'text':
					...
					if ( ! empty( $field['description'] ) ) {
						echo '<span class="description">' . dhecho( $field['description'] ) . '</span>';
					}
				...
				case 'checkbox':
					...
					if ( ! empty( $field['description'] ) ) echo '<span class="description">' . dhecho( $field['description'] ) . '</span>';
				...
				case 'widgetised_sidebars':
					...
					echo '<option value="">' . __('Select a sidebar...',DH_DOMAIN) . '</option>';
				...
				case 'gallery':
						...
						jQuery(document).ready(function($) {
							$('.dh-meta-gallery-select').on('click',function(e){
								...
								dh_meta_gallery_frame = wp.media({
									title: '<?php echo __('Add Images to Gallery',DH_DOMAIN)?>',
									button: {
										text: '<?php echo __('Add to Gallery',DH_DOMAIN)?>',
									},
									library: { type: 'image' },
									multiple: true
								});
											...
											dh_meta_gallery_list.append('\
												<li data-id="' + attachment.id +'">\
													<div class="thumbnail">\
														...
														<a href="#" title="<?php echo __('Delete',DH_DOMAIN)?>"><?php echo __('x',DH_DOMAIN)?></a></li>\
													</div>\
												</li>'
											);
										}
							...
							?>
								<li data-id="<?php echo esc_attr( $attachment_id ) ?>">
									<div class="thumbnail">
										...
										<a title="<?php echo __('Delete',DH_DOMAIN) ?>" href="#"><?php echo __('x',DH_DOMAIN) ?></a>
									</div>						
								</li>
							<?php
								endif;
					...
					echo '<input type="button" class="button button-primary dh-meta-gallery-select" name="' . $field['id'] . '_button_upload" id="' . $field['id'] . '_upload" value="' . __('Add Gallery Images',DH_DOMAIN) . '" /> ';
					if ( ! empty( $field['description'] ) ) {
						echo '<span class="description">' . dhecho( $field['description'] ) . '</span>';
					}
					echo '</div>';
				...
				case 'media':
					...
					$btn_text = !empty(  $field['value'] ) ? __( 'Change Media', DH_DOMAIN ) : __( 'Select Media', DH_DOMAIN );
					echo '<div  class="dh-meta-box-field ' . esc_attr( $field['id'] ) . '_field">';
					echo '<label for="' . esc_attr( $field['id'] ) . '">' . esc_html( $field['label'] ) . '</label>';
					...
					echo '<input type="button" class="button" name="' . $field['id'] . '_button_clear" id="' . $field['id'] . '_clear" value="' . __( 'Clear', DH_DOMAIN ) . '" />';				
					if ( ! empty( $field['description'] ) ) {
						echo '<span class="description">' . dhecho( $field['description'] ) . '</span>';
					}
					echo '</div>';
						...
						jQuery(document).ready(function($) {
							...
							$('#<?php echo esc_attr($field['id']) ?>_upload').on('click', function(event) {
								...
								var dh_<?php echo esc_attr($field['id']); ?>_media_frame = wp.media.frames.wp_media_frame = wp.media({
									title: "<?php echo __( 'Select or Upload your Media', DH_DOMAIN ); ?>",
									button: {
								...
								dh_<?php echo esc_attr($field['id']); ?>_media_frame.on('select', function(){
									...
									$this.attr('value', '<?php echo __( 'Change Media', DH_DOMAIN ); ?>');
							...
							$('#<?php echo esc_attr($field['id']) ?>_clear').on('click', function(event) {
								var $this = $(this);
								$this.hide();
								$('#<?php echo esc_attr($field['id']) ?>_upload').attr('value', '<?php echo __( 'Select Media', DH_DOMAIN ); ?>');
				...
				case 'image':
					...
					echo '<input type="button" class="button button-primary" name="' . $field['id'] . '_button_upload" id="' . $field['id'] . '_upload" value="' . $btn_text . '" /> ';
					echo '<input type="button" class="button" name="' . $field['id'] . '_button_clear" id="' . $field['id'] . '_clear" value="' . __( 'Clear Image', DH_DOMAIN ) . '" />';
					if ( ! empty( $field['description'] ) ) {
						echo '<span class="description">' . dhecho( $field['description'] ) . '</span>';
					}
					?>
