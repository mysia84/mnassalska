	...
	class DHController {
		...
		public function facebook_init(){
			...
			if( !isset( $_REQUEST['fb_response'] ) || !isset( $_REQUEST['fb_user'] )) {
				die(json_encode(array('error' => __('Authenication required.', DH_DOMAIN))));
			}
			$redirect = '';
			...
			if( !$fb_userid ) {
				die(json_encode(array('error' => __('Please connect your facebook account.', DH_DOMAIN))));
			}
			...
			if( !$user_ID ){
				...
				if( !$user_ID )
				{
					if ( !get_option( 'users_can_register' )) {
						die( json_encode( array( 'error' => __('Registration is not open at this time. Please come back later.', DH_DOMAIN) )));
					}
					if (dh_get_theme_option('facebook_login',0) == 0) {
						die( json_encode( array( 'error' => __('Registration using Facebook is not currently allowed. Please use our Register page', DH_DOMAIN) )));
					}
					extract( $fb_user );
					$display_name = $name;
					...
					if( empty( $verified ) || !$verified ) {
						die(json_encode(array('error' => __('Your facebook account is not verified. You have to verify your account before proceed login or registering on this site.', DH_DOMAIN))));
					}
			
					$user_email = $email;
					if ( empty( $user_email )) {
						die(json_encode(array('error' => __('Please click again to login with Facebook and allow the application to use your email address', DH_DOMAIN))));
					}
			
					if( empty( $name )) {
						die(json_encode(array('error' => 'empty_name', __('We didn\'t find your name. Please complete your facebook account before proceeding.', DH_DOMAIN))));
					}
			...
			die( json_encode( array( 'loggedin' => true, 'type' => $logintype, 'url' => $redirect, 'siteUrl' => home_url(), 'message' => __( 'Login successful, redirecting...',DH_DOMAIN ) )));
		...
		public function login(){
			...
			if ( is_wp_error( $user_signon ) ){
				...
				wp_send_json(array('loggedin'=>true, 'redirecturl' => $redirecturl, 'message'=> '<span class="success-response"><i class="fa fa-check-circle"></i> ' . __( 'Login successful, redirecting...',DH_DOMAIN ) . '</span>' ));
			}
			die();
		}
		public function register(){
			if( !check_ajax_referer('dh-ajax-register-nonce', 'security', false) ) {
				$result = array(
					'success' => false,
					'message' => '<span class="error-response">'.__( 'Your session is expired or you submitted an invalid form.', DH_DOMAIN ).'</span>',
				);
				wp_send_json($result);
			}
			if(get_option( 'users_can_register' )){
				...
				if ( is_wp_error( $errors ) ) {
					...
					$result = array(
						'success'     => true,
						'message'	=> '<span class="success-response"><i class="fa fa-check-circle"></i> '.__( 'Registration complete.', DH_DOMAIN ).'</span>',
				...
				$result = array(
					'success' => false,
					'message'   =>__( 'Not allow register in site.', DH_DOMAIN ),
				);
			}
			wp_send_json($result);
		}
		...
		protected function _register_new_user($user_login, $user_email, $user_password='', $cuser_password=''){
			...
			if ( $sanitized_user_login == '' ) {
				$errors->add( 'empty_username', __( 'Please enter a username.', DH_DOMAIN ) );
			} elseif ( ! validate_username( $user_login ) ) {
				$errors->add( 'invalid_username', __( 'This username is invalid because it uses illegal characters. Please enter a valid username.', DH_DOMAIN ) );
				$sanitized_user_login = '';
			} elseif ( username_exists( $sanitized_user_login ) ) {
				$errors->add( 'username_exists', __( 'This username is already registered. Please choose another one.', DH_DOMAIN ) );
			}
			
			// Check the email address
			if ( $user_email == '' ) {
				$errors->add( 'empty_email', __( 'Please type your email address.', DH_DOMAIN ) );
			} elseif ( ! is_email( $user_email ) ) {
				$errors->add( 'invalid_email', __( 'The email address isn\'t correct.', DH_DOMAIN ) );
				$user_email = '';
			} elseif ( email_exists( $user_email ) ) {
				$errors->add( 'email_exists', __( 'This email is already registered, please choose another one.', DH_DOMAIN ) );
			}
			//Check the password
			...
			if(empty($user_password)){
				...
				if(strlen($user_password) < 6){
					$errors->add( 'minlength_password', __( 'Password must be 6 character long.', DH_DOMAIN ) );
				}elseif (empty($cuser_password)){
					$errors->add( 'not_cpassword', __( 'Not see password confirmation field.', DH_DOMAIN ) );
				}elseif ($user_password != $cuser_password){
					$errors->add( 'unequal_password', __( 'Passwords do not match.', DH_DOMAIN ) );
				}
			}
			...
			if ( ! $user_id ) {
				$errors->add( 'registerfail', __( 'Couldn\'t register you... please contact the site administrator', DH_DOMAIN ) );
				return $errors;
			}
			...
			// user email
			$subject = sprintf(__('Welcome to [%1$s]',DH_DOMAIN),$blogname);
			$to = $user->email;
...
%4$s',DH_DOMAIN);
		...
		public function register_priv(){
			...
			wp_send_json(array('success'=>true, 'message'=> sprintf(__('You are already logged in. Please <a href="#" onclick="%s">refresh</a> page',DH_DOMAIN),$link)));
			die();
		}
		
		public function login_priv(){
			$link = "javascript:window.location.reload();return false;";
			wp_send_json(array('loggedin'=>true, 'message'=> sprintf(__('You are already logged in. Please <a href="#" onclick="%s">refresh</a> page',DH_DOMAIN),$link)));
			die();
		}
		...
		public function lostpassword(){
			...
			if ( isset($_POST) ) {
				...
				*/
				...
				if ( ! $allow ) {
					echo '<span class="error-response">' . __('Password reset is not allowed for this user') . '</span>';
					die();
				}
				...
				*/
				...
				if ( $message && !wp_mail( $user_email, wp_specialchars_decode( $title ), $message ) ) {
					echo '<span class="error-response">' . __("Failure!", DH_DOMAIN);
					echo __('The e-mail could not be sent.');
					echo "</span>";
					die();
				} else {
					echo '<span class="success-response"><i class="fa fa-check-circle"></i> ' . __("Email successfully sent!", DH_DOMAIN)."</span>";
					die();
				}
			}
			die();
		...
		protected function _mailchimp_subscribe($is_ajax = false) {
			...
			if(!wp_verify_nonce( $nonce, 'mailchimp_subscribe_nonce' )){
				if($is_ajax)
					wp_send_json(array( 'success' => false, 'message' => '<span class="error-response"><i class="fa fa-times-circle"></i> ' . __('Verify Nonce Failed',DH_DOMAIN) . '</span>' ));
				else
					wp_die(__('Verify Nonce Failed',DH_DOMAIN));
			}
			if(!filter_var(@$data['email'], FILTER_VALIDATE_EMAIL)){
				if($is_ajax)
					wp_send_json(array( 'success' => false, 'message' => '<span class="error-response"><i class="fa fa-times-circle"></i> ' . __('Please enter a valid email address',DH_DOMAIN) . '</span>' ));
				else
					wp_die(__('Invalid Email',DH_DOMAIN));
			}
			...
			if($is_ajax){
				if($success == 1){
					wp_send_json(array( 'success' => true, 'message' => '<span class="success-response"><i class="fa fa-check-circle"></i> ' . __('<strong>Well done!</strong> Subscribe to our Newsletter successful!',DH_DOMAIN) . '</span>' ));
				}else{
					wp_send_json(array( 'success' => false, 'message' => '<span class="error-response"><i class="fa fa-times-circle"></i> ' . __('Can not subscribe our Newsletter',DH_DOMAIN) . '</span>' ));
				}
			}else{
		...
		public function search() {
			...
			if ( empty( $posts ) ) {
				...
				$output .= __( 'No results matched!', DH_DOMAIN );
				$output .= '</div>';
			} else {
				$post_types = array();
				...
				$output .= '<div class="search-view-all"><a href="' . home_url( '?s=' . $_REQUEST['s'] ) . '">' .
					 __( 'View all results', DH_DOMAIN ) . '</a>';
			}
