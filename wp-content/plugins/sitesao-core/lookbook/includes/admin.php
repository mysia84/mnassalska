...
function  dhlookbook_product_lookbook_description() {
	echo wpautop( __( 'Product lookbooks for your store can be managed here. To change the order of lookbooks on the front-end you can drag and drop to sort them. To see more lookbooks listed click the "screen options" link at the top of the page.', DH_DOMAIN ) );
}
...
?>
	<div class="form-field">
		<label><?php _e( 'Small title', DH_DOMAIN ); ?></label>
		<input type="text" size="40" value="" id="small_title" name="small_title">
	</div>
	<div class="form-field">
		<label><?php _e( 'Thumbnail', DH_DOMAIN ); ?></label>
		...
		<div style="line-height:60px;">
			...
			<button type="submit" class="upload_image_button button"><?php _e('Upload/Add image', DH_DOMAIN); ?></button>
			<button type="submit" class="remove_image_button button"><?php _e('Remove image', DH_DOMAIN); ?></button>
		</div>
		<script type="text/javascript">
			...
			jQuery(document).on( 'click', '.upload_image_button', function( event ){
				...
				file_frame = wp.media.frames.downloadable_file = wp.media({
					title: '<?php _e( 'Choose an image', DH_DOMAIN ); ?>',
					button: {
						text: '<?php _e( 'Use image', DH_DOMAIN ); ?>',
					},
					multiple: false
				});
	...
	<div class="form-field">
		<label><?php _e( 'Thumbnail Align', DH_DOMAIN ); ?></label>
		<select class="postform" name="lookbook_thumbnail_align">
			<option value="left"><?php esc_html_e('Left',DH_DOMAIN)?></option>
			<option value="right"><?php esc_html_e('Right',DH_DOMAIN)?></option>
		</select>
		<div class="clear"></div>
	</div>
<?php 
...
function  dhlookbook_edit_lookbook_thumbnail($term, $taxonomy ){
	...
	?>
	<tr class="form-field">
		<th scope="row" valign="top"><label><?php _e( 'Small title', DH_DOMAIN ); ?></label></th>
		<td><input type="text" size="40" value="<?php echo esc_attr($small_title)?>" id="small_title" name="small_title"></td>
	</tr>
	<tr class="form-field">
		<th scope="row" valign="top"><label><?php _e('Thumbnail', DH_DOMAIN); ?></label></th>
		<td>
	...
	<tr class="form-field">
		<th scope="row" valign="top"><label><?php _e('Thumbnail Align', DH_DOMAIN); ?></label></th>
		<td>
			<select class="postform" name="lookbook_thumbnail_align">
				<option value="left" <?php selected($lookbook_thumbnail_align,'left')?>><?php esc_html_e('Left',DH_DOMAIN)?></option>
				<option value="right" <?php selected($lookbook_thumbnail_align,'right')?>><?php esc_html_e('Right',DH_DOMAIN)?></option>
			</select>
		</td>
	</tr>
<?php 
...
function  dhlookbook_product_lookbook_columns( $columns ) {
	...
	$new_columns['thumb'] = __( 'Image', DH_DOMAIN );
...
function  dhlookbook_permalink_settings_add(){
	add_settings_field(
		...
		__( 'Product lookbook base', DH_DOMAIN ), 	// setting title
...
function  dhlookbook_product_lookbook_slug_input(){
	$permalinks = get_option( 'woocommerce_permalinks' );
	?>
	<input name="dhlookbook_product_lookbook_slug" type="text" class="regular-text code" value="<?php if ( isset( $permalinks['lookbook_base'] ) ) echo esc_attr( $permalinks['lookbook_base'] ); ?>" placeholder="<?php echo _x('product-lookbook', 'slug', DH_DOMAIN) ?>" />
	<?php
}
