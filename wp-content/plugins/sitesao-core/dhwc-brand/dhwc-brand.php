...
if (dhwc_is_active() && !class_exists('DHWC_Brand')){
	class DHWC_Brand {
		...
		function dhwc_init_taxonomy(){
			...
			$product_brand_slug = empty( $permalinks['brand_base'] ) ? _x( 'product-brand', 'slug', DH_DOMAIN ) : $permalinks['brand_base'];
			...
			register_taxonomy ( 'product_brand', 
				...
				apply_filters ( 'dhwc_taxonomy_args_product_brand', array (
					...
					'label' => __ ( 'Product Brands', DH_DOMAIN ),
					'labels' => array (
							'name' => __ ( 'Product Brands', DH_DOMAIN ),
							'singular_name' => __ ( 'Product Brand', DH_DOMAIN ),
							'menu_name' => _x ( 'Brands', 'Admin menu name', DH_DOMAIN ),
							'search_items' => __ ( 'Search Product Brands', DH_DOMAIN ),
							'all_items' => __ ( 'All Product Brands', DH_DOMAIN ),
							'parent_item' => __ ( 'Parent Product Brand', DH_DOMAIN ),
							'parent_item_colon' => __ ( 'Parent Product Brand:', DH_DOMAIN ),
							'edit_item' => __ ( 'Edit Product Brand', DH_DOMAIN ),
							'update_item' => __ ( 'Update Product Brand', DH_DOMAIN ),
							'add_new_item' => __ ( 'Add New Product Brand', DH_DOMAIN ),
							'new_item_name' => __ ( 'New Product Brand Name', DH_DOMAIN ) 
					),
					'show_ui' => true,
		...
		*/
	    ...
	    function dhwc_add_prouduct_brand_meta(){
			...
			if ( is_singular( 'product' ) ) {
				echo apply_filters('dhwc_prouduct_brand_meta',dhwc_get_brands( $post->ID, ', ', ' <span class="posted_in">' . __('Brand:', DH_DOMAIN).' ', '.</span>' ));
			}
	    }
