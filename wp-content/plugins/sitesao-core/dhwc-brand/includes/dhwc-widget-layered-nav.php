...
class DHWC_Widget_Layered_Nav extends WP_Widget {
	...
	public function __construct(){
		$widget_options = array( 'classname' => 'woocommerce widget_layered_nav dhwc_widget_layered_nav', 'description' => __( 'Shows product brands in a widget which lets you narrow down the list of products when viewing product categories',DH_DOMAIN) );
		$control_options = array();
		parent::__construct('dhwc_widget_layered_nav',__('DHWOO Product Brand Layered Nav',DH_DOMAIN), $widget_options, $control_options);
	}
		...
		if ( count( $terms ) > 0 ) {
			...
			if ( $display_type == 'dropdown' ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					echo '<option value="">' . __( 'Any brand', DH_DOMAIN ) .'</option>';
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( empty( $new_instance['title'] ) )
			$new_instance['title'] = __('Brands',DH_DOMAIN);
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'display_type' ); ?>"><?php _e( 'Display Type:', DH_DOMAIN ) ?></label>
		<select id="<?php echo esc_attr( $this->get_field_id( 'display_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'display_type' ) ); ?>">
			<option value="list" <?php selected( $instance['display_type'], 'list' ); ?>><?php _e( 'List', DH_DOMAIN ); ?></option>
			<option value="dropdown" <?php selected( $instance['display_type'], 'dropdown' ); ?>><?php _e( 'Dropdown', DH_DOMAIN ); ?></option>
		</select></p>
